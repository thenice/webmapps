    ...
    if (self = [super initWithFrame:aFrame]) {
        ...
        with (_DOMMapElement.style) {
            ...
            height = "100%";
				...
				_DOMMapElement.id = "map-container";
...
- (void)createMap
{
    ...
    function endDrag(ev)
    {
		...
		// Map mouse events handled here
		
		// drag work around
    var dragNode = _DOMMapElement.firstChild.firstChild;
    GEvent.addDomListener(dragNode, 'mousedown', startDrag);

		// handle clicks -- makeazatest!!!
		
		/*GEvent.addDomListener(_DOMMapElement, 'click', function(ev) {
			var current = _gMap.fromContainerPixelToLatLng(new GPoint(ev.clientX, ev.clientY));
			var lng = 0;
			var lat = 0;
			var zoom = _gMap.getZoom();
			
			alert("current: " + current.lat() + "," + current.lng() + " @zoom: " + _gMap.getZoom());
